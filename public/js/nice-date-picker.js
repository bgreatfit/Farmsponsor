...
(function (global, factory) {
    ...
    niceDatePicker.prototype.buildUi = function (year, month) {
        ...
        this.dayWords = [['一', '二', '三', '四', '五', '六', '日'], ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun']];
        this.enMonthsWords = ['January', 'Febuary', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'];
        ...
        for (var i = 0; i < this.monthData.date.length; i++) {
            ...
            html += '<td class="' + this.monthData.date[i].styleCls + '" data-date="' + this.enMonthsWords[this.monthData.month - 1]  + ' ' + this.monthData.date[i].showDate + ', ' +   this.monthData.year + '">' + this.monthData.date[i].showDate + '</td>';
            if (i % 7 === 6) {
                html += '</tr>';
            }
        }
